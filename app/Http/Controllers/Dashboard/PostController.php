...
class PostController extends Controller
{
    ...
    public function update(UpdatePostRequest $request, Post $post)
    {
        ...
        if ($request->hasFile('thumbnail')) {
            $thumbnail = $request->file('thumbnail');
            $thumbnailName = time(). '.'. $thumbnail->getClientOriginalExtension();
            $destinationPath = Storage::disk('public')->path('thumbnail');
            $thumbnail->move($destinationPath, $thumbnailName);
            $validated['thumbnail'] = $thumbnailName;

            if ($post->thumbnail) {
                Storage::disk('public')->delete('thumbnail/' . $post->thumbnail);
            }
        }

        $post->update($validated);

        return redirect()->route('dashboard.pages.posts.index')->with('success', 'Post updated successfully');
    }
